...
static int get_field( const char* header,
                      const uint8_t* table,
                      field_data* fd) {
    fd->datatype = table[ fd->field_id ];

    switch ( fd->datatype ) {

        case SEGY_SIGNED_INTEGER_4_BYTE:
            memcpy( &(fd->value.i32), header + (fd->field_id - 1), formatsize( fd->datatype ) );
            fd->value.i32 = be32toh( fd->value.i32 );
            return SEGY_OK;

        case SEGY_SIGNED_SHORT_2_BYTE:
            memcpy( &(fd->value.i16), header + (fd->field_id - 1), formatsize( fd->datatype ) );
            fd->value.i16 = be16toh( fd->value.i16 );
            return SEGY_OK;

        case SEGY_SIGNED_CHAR_1_BYTE:
            memcpy( &(fd->value.i8), header + (fd->field_id - 1), formatsize( fd->datatype ) );
            return SEGY_OK;

        case SEGY_UNSIGNED_SHORT_2_BYTE:
            memcpy( &(fd->value.u16), header + (fd->field_id - 1), formatsize( fd->datatype ) );
            fd->value.u16 = be16toh( fd->value.u16 );
            return SEGY_OK;

        case SEGY_UNSIGNED_CHAR_1_BYTE:
            memcpy( &(fd->value.u8), header + (fd->field_id - 1), formatsize( fd->datatype ) );
            return SEGY_OK;
...
int segy_get_field( const char* traceheader, int field, int* f ) {
    ...
    field_data fd;
    fd.field_id = field;
    int err = get_field( traceheader, tr_field_type, &fd );
...
int segy_get_bfield( const char* binheader, int field, int32_t* f ) {
    ...
    field_data fd;
    fd.field_id = field;
    int err = get_field( binheader, bin_field_type, &fd );
    if ( err != SEGY_OK ) return err;
    err = fd_get_int( &fd, f );
    return err;
}
...
static int set_field( char* header,
    ...
    switch ( w_fd.datatype ) {
        ...
        case SEGY_SIGNED_INTEGER_4_BYTE:
            ...
            memcpy( header + (w_fd.field_id - 1), &(w_fd.value.i32), formatsize( w_fd.datatype ));
            return SEGY_OK;

        case SEGY_SIGNED_SHORT_2_BYTE:
            w_fd.value.i16 = htobe16( w_fd.value.i16 );
            memcpy( header + (w_fd.field_id - 1), &(w_fd.value.i16), formatsize( w_fd.datatype ));
            return SEGY_OK;

        case SEGY_SIGNED_CHAR_1_BYTE:
            memcpy( header + (w_fd.field_id - 1), &(w_fd.value.i8), formatsize( w_fd.datatype ));
            return SEGY_OK;

        case SEGY_UNSIGNED_INTEGER_4_BYTE:
            w_fd.value.u32 = htobe32( w_fd.value.u32 );
            memcpy( header + (w_fd.field_id - 1), &(w_fd.value.u32), formatsize( w_fd.datatype ));
            return SEGY_OK;

        case SEGY_UNSIGNED_SHORT_2_BYTE:
            w_fd.value.u16 = htobe16( w_fd.value.u16 );
            memcpy( header + (w_fd.field_id - 1), &(w_fd.value.u16), formatsize( w_fd.datatype ));
            return SEGY_OK;

        case SEGY_UNSIGNED_CHAR_1_BYTE:
            memcpy( header + (w_fd.field_id - 1), &(w_fd.value.u8), formatsize( w_fd.datatype ));
            return SEGY_OK;
...
int segy_field_forall( segy_file* fp,
    ...
    if( fp->addr ) {
        for( int i = start; slicelen > 0; i += step, ++buf, --slicelen ) {
            ...
            field_data fd;
            fd.field_id = field;
            err = get_field( fp->cur, tr_field_type, &fd );
            if( err != 0 ) return err;
            err = fd_get_int( &fd, &f );
            if( err != 0 ) return err;
    ...
    for( int i = start; slicelen > 0; i += step, ++buf, --slicelen ) {
        ...
        field_data fd;
        fd.field_id = field;
        err = get_field( header, tr_field_type, &fd );
        if( err != 0 ) return err;
        err = fd_get_int( &fd, &f );
        if( err != 0 ) return err;
        if (lsb) f = bswap_header_word(f, formatsize( fd.datatype ));
