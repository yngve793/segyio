...
SEGY_FORMAT segy_entry_type_to_format( SEGY_ENTRY_TYPE entry_type ) {
    switch( entry_type ) {
        case SEGY_ENTRY_TYPE_INT2:
        case SEGY_ENTRY_TYPE_TIME2:
        case SEGY_ENTRY_TYPE_SCALE6_EXP:
            return SEGY_SIGNED_SHORT_2_BYTE;
...
static int set_field( char* header,
    ...
    uint8_t datatype = segy_entry_type_to_format( mapping[offset].entry_type );
...
int segy_field_forall( segy_datasource* ds,
    ...
    for( int i = start; slicelen > 0; i += step, ++buf, --slicelen ) {
        ...
        err = segy_get_tracefield_int( header, field, &f );
        if( err != 0 ) return err;
        uint8_t entry_type = traceheader_default_map[zfield].entry_type;
        uint8_t datatype = segy_entry_type_to_format( entry_type );
